...
. /root/user.name/Scale.properties
...
then
		...
		then
			...
			then
				...
				Instance_ID=`grep $Off_line_to_UP /root/user.name/Node_details | awk '{print $2}'`
				rregion=`grep $Off_line_to_UP /root/user.name/Node_details | awk '{print $3}'`
...
User Name
EOL
mail -s "Torque Node State chng to IN" user.name@g.gmailom < outchk
...
egrep -v `echo $in_var_onlin_nod` AA_1 > AA_2
				...
				instance_ID=`grep $node_going_scale /root/user.name/Node_details | awk '{print $2}'`
				Region=`grep $node_going_scale  /root/user.name/Node_details | awk '{print $3}'`
...
####################################		ALL DOWN PROCESS		##########################
		...
		Instance_ID=`grep $NODE_NAME /root/user.name/Node_details | awk '{print $2}'`
		rregion=`grep $NODE_NAME /root/user.name/Node_details | awk '{print $3}'`
